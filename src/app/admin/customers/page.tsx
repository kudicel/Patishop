...
export default async function AdminCustomersPage() {
  ...
  return (
    <div className="p-6 lg:p-8">
      ...
      <p className="text-[#7ecad6] text-sm mb-8">Toplam {customers.length} kayıtlı müşteri</p>

      <div className="rounded-2xl border border-white/[0.08] overflow-hidden">
        {customers.length === 0 ? (
          <p className="px-6 py-12 text-[#7ecad6] text-sm text-center">Henüz kayıtlı müşteri yok.</p>
        ) : (
              ...
              <tbody>
                {customers.map(c => (
                  <tr key={c.id}
                    ...
                    <td className="px-6 py-3 text-[#7ecad6]">{c.email}</td>
                    <td className="px-6 py-3 text-center">
                      <span className={`px-2.5 py-1 rounded-full text-xs font-bold
                        ${c._count.orders > 0
                          ? 'bg-[rgba(6,182,212,0.12)] text-[#06b6d4]'
                          : 'bg-white/[0.06] text-[#7ecad6]'}`}>
                        {c._count.orders}
                      </span>
                    </td>
