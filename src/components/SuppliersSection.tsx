  ...
  return (
    <section id="suppliers" className="px-6 lg:px-12 pb-20">
      <div className="mb-10">
        ...
        <p className="text-[#c4a896] max-w-2xl">{t(country, 'sup_p')}</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        {SUPPLIERS.map(s => (
          <article key={s.name} className="rounded-[1.75rem] border border-[rgba(255,154,60,0.1)] bg-white/[0.04] p-7">
            ...
            <p className="text-[#c4a896] text-sm leading-relaxed mb-4">{t(country, s.descKey)}</p>
            <ul className="space-y-1.5">
              {s.featureKeys.map(fk => (
                <li key={fk} className="flex items-start gap-2 text-sm text-[#c4a896]">
                  <span className="text-[#ff9a3c] flex-shrink-0">✓</span>{t(country, fk)}
                </li>
              ))}
            </ul>
          </article>
